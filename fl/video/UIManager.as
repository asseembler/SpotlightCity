package fl.video
{
   ...
   public class UIManager
   {
      ...
      public function UIManager(param1:FLVPlayback)
      {
         ...
         this._skinFadingMaxTime = flvplayback_internal::SKIN_FADING_MAX_TIME_DEFAULT;
         this._skinAutoHideMotionTimeout = flvplayback_internal::SKIN_AUTO_HIDE_MOTION_TIMEOUT_DEFAULT;
         ...
         this._skinAutoHideTimer = new Timer(flvplayback_internal::SKIN_AUTO_HIDE_INTERVAL);
         this._skinAutoHideTimer.addEventListener(TimerEvent.TIMER,this.skinAutoHideHitTest);
         this._skinFadingTimer = new Timer(flvplayback_internal::SKIN_FADING_INTERVAL);
         ...
         switch(param1.type)
         {
            ...
            case MetadataEvent.METADATA_RECEIVED:
               ...
               if(this._vc.getVideoPlayer(param1.vp).isRTMP)
               {
                  ...
                  while(_loc3_ <= VOLUME_BAR)
                  {
                     if((_loc5_ = this.controls[_loc3_]) != null)
                     {
                        if((_loc6_ = this.ctrlDataDict[_loc5_]).progress_mc != null)
                        {
                           this.positionBar(_loc5_,"progress",this._progressPercent);
            ...
            case VideoEvent.PLAYHEAD_UPDATE:
               ...
               {
                  if((_loc10_ = (_loc4_ = VideoEvent(param1)).playheadTime / this._vc.totalTime * 100) < 0)
                  {
                     _loc10_ = 0;
                  }
                  else if(_loc10_ > 100)
            ...
            case VideoProgressEvent.PROGRESS:
               ...
               _loc9_ = (_loc8_ = this._vc.videoPlayerStates[param1.vp]).minProgressPercent;
               ...
               {
                  if((_loc11_ = this._vc.playheadTime / this._vc.totalTime * 100) > this._progressPercent)
                  {
      ...
      flvplayback_internal function handleLoad(param1:Event) : void
      {
         ...
         try
         {
            ...
            while(i < this.layout_mc.numChildren)
            {
               ...
               index = flvplayback_internal::layoutNameToIndexMappings[dispObj.name];
               if(!isNaN(index))
               {
         ...
         {
            flvplayback_internal::_vc.skinError(err.message);
            removeSkin();
         }
      }
      ...
      flvplayback_internal function layoutSkin() : void
      {
         ...
         if(this._fullScreen && this._fullScreenTakeOver && this.border_mc != null)
         {
            ...
            if(borderRect.width > 0 && borderRect.height > 0)
            {
               if(borderRect.x < 0)
               {
                  this.placeholderLeft += this.videoLeft - borderRect.x;
                  forceSkinAutoHide = true;
               }
               if(borderRect.x + borderRect.width > this._vc.registrationWidth)
               {
                  this.placeholderRight += borderRect.x + borderRect.width - this.videoRight;
                  forceSkinAutoHide = true;
               }
               if(borderRect.y < 0)
               {
                  this.placeholderTop += this.videoTop - borderRect.y;
                  forceSkinAutoHide = true;
               }
               if(borderRect.y + borderRect.height > this._vc.registrationHeight)
               {
                  this.placeholderBottom += borderRect.y + borderRect.height - this.videoBottom;
                  forceSkinAutoHide = true;
               }
         ...
         try
         {
            if(!isNaN(this.layout_mc["minWidth"]))
            {
               ...
               {
                  this.videoLeft -= (minWidth - vidWidth) / 2;
         ...
         try
         {
            if(!isNaN(this.layout_mc["minHeight"]))
            {
               ...
               {
                  this.videoTop -= (minHeight - vidHeight) / 2;
      ...
      flvplayback_internal function bitmapCopyBorder() : void
      {
         ...
         if(this.borderPrevRect == null || !this.borderPrevRect.equals(_loc1_))
         {
            ...
            else
            {
               ...
               {
                  _loc8_ += this.borderScale9Rects[3].width;
               }
               if(this.borderScale9Rects[5] != null)
               {
                  _loc8_ += this.borderScale9Rects[5].width;
               }
               _loc9_ = 0;
               if(this.borderScale9Rects[1] != null)
               {
                  _loc9_ += this.borderScale9Rects[1].height;
               }
               if(this.borderScale9Rects[7] != null)
               {
                  _loc9_ += this.borderScale9Rects[7].height;
               }
               _loc10_ = 0;
      ...
      flvplayback_internal function setSkin(param1:int, param2:DisplayObject) : void
      {
         ...
         else
         {
            switch(param1)
            {
               ...
               case SEEK_BAR:
               case VOLUME_BAR:
                  _loc5_ = flvplayback_internal::skinClassPrefixes[param1];
      ...
      flvplayback_internal function setTwoButtonHolderSkin(param1:int, param2:int, param3:String, param4:int, param5:String, param6:DisplayObject) : Sprite
      {
         ...
         (_loc7_ = this.setupButtonSkin(param2,param6)).name = param3;
         _loc7_.visible = true;
         _loc8_.addChild(_loc7_);
         (_loc7_ = this.setupButtonSkin(param4,param6)).name = param5;
      ...
      {
         var _loc3_:String = flvplayback_internal::skinClassPrefixes[param1];
         if(_loc3_ == null)
         {
            return null;
         }
      ...
      flvplayback_internal function skinButtonControl(param1:Object) : void
      {
         ...
         {
            ctrlData.state_mc[NORMAL_STATE] = this.setupButtonSkinState(ctrl,ctrl,flvplayback_internal::buttonSkinLinkageIDs[NORMAL_STATE],null);
         }
         if(ctrlData.enabled && this._controlsEnabled)
         {
            if(ctrlData.state_mc[ctrlData.state] == undefined)
            {
               ctrlData.state_mc[ctrlData.state] = this.setupButtonSkinState(ctrl,ctrl,flvplayback_internal::buttonSkinLinkageIDs[ctrlData.state],ctrlData.state_mc[NORMAL_STATE]);
            }
      ...
      flvplayback_internal function positionHandle(param1:Sprite) : void
      {
         ...
         var _loc5_:Number;
         var _loc6_:Number = (_loc5_ = !!isNaN(_loc2_.origWidth) ? Number(param1.width) : Number(_loc2_.origWidth)) - _loc4_.rightMargin - _loc4_.leftMargin;
      ...
      flvplayback_internal function setEnabledAndVisibleForState(param1:int, param2:String) : void
      {
         ...
         this._vc.activeVideoPlayerIndex = this._vc.visibleVideoPlayerIndex;
         var _loc4_:String;
         if((_loc4_ = param2) == VideoState.BUFFERING && !this._bufferingOn)
         {
            _loc4_ = VideoState.PLAYING;
         }
         var _loc5_:Sprite;
         if((_loc5_ = this.controls[param1]) == null)
         {
            return;
         }
      ...
      flvplayback_internal function hookUpCustomComponents() : void
      {
         ...
         i = 0;
         for(; i < this._vc.parent.numChildren; i++)
         {
                  ...
                  case this.controls[VOLUME_BAR_HANDLE]:
                     if(k != Keyboard.TAB && (k == Keyboard.UP || k == Keyboard.DOWN || k == Keyboard.LEFT || k == Keyboard.RIGHT || k == Keyboard.PAGE_UP || k == Keyboard.PAGE_DOWN || k == Keyboard.HOME || k == Keyboard.END || !isNaN(ka) && kaBool))
                     {
                        ...
                        if(event.target == this.controls[SEEK_BAR_HANDLE])
                        {
                           ...
                           {
                              percent -= this._vc.seekBarScrubTolerance * 2;
                           ...
                           else if(k == Keyboard.RIGHT || k == Keyboard.UP)
                           {
                              ...
                              {
                                 percent += this._vc.seekBarScrubTolerance;
